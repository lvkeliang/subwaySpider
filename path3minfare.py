...
for i, start_row in dfState.iterrows():
    for j, end_row in df.iterrows():
        ...
        path, cost, fare = shortest_path(start_row['lat'], start_row['lon'], end_row['lat'], end_row['lon'], time_weight=0, fare_weight=1)
...
# 将结果保存到新的Excel文件中
results.to_excel('question3_minfare_resultst.xlsx', index=False)
...
for start_name in dfState['location']:
    for end_name in df['location']:
        if start_name != end_name:
            ...
            # 计算路径
            path, _ , _= shortest_path(start_lat, start_lon, end_lat, end_lon, 0, 1)

            # 将路径保存到对应的单元格中
            path_df.at[start_name, end_name] = path

# 将路径矩阵保存到新的Excel文件中
path_df.to_excel('question3_minfare_matrix.xlsx')

print('路径矩阵已保存到 question3_minfare_matrix.xlsx 文件中。')